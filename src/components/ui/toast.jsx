...
const ToastViewport = React.forwardRef(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ...
    className={`fixed top-0 z-100 flex max-h-screen w-full flex-col-reverse p-4 
                sm:bottom-0 sm:right-0 sm:top-auto sm:flex-col md:max-w-[420px] ${props.className || ""}`}
    {...props}
  />
));
...
const Toast = React.forwardRef(({ className, variant, ...props }, ref) => {
  return (
    <ToastPrimitives.Root
      ref={ref}
      className={`${toastVariants({ variant })} ${props.className || ""}`}
      {...props}
    />
...
const ToastClose = React.forwardRef(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={`absolute right-2 top-2 rounded-md p-1 text-foreground/50 opacity-0 transition-opacity 
                ...
                focus:group-[.destructive]:ring-offset-red-600 ${props.className || ""}`}
    toast-close=""
    {...props}
  >
    <X className="h-4 w-4" />
...
const ToastDescription = React.forwardRef(({ className, ...props }, ref) => (
  ...
  ref={ref}
  className={`text-sm opacity-90 ${props.className || ""}`}
  {...props}
  />
));
